...
import { useValidateCoupon, useApplyCoupon } from '../../tanstack/useCoupons';
import {
  FiShoppingCart,
  FiTrash2,
  FiPlus,
...
const Cart = () => {
  ...
  // Memoize cart data
  const cart = useMemo(() => cartData?.data || cartData, [cartData]);
  ...
  const formatVariantOptions = useCallback((variantOptions: any) => {
  ...
  const handleQuantityChange = useCallback(
    async (skuId: string, newQuantity: number) => {
      if (newQuantity < 1) {
        ...
        <div className="space-y-8">
                      ...
                      <div className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="hidden lg:flex items-center space-x-1">
                            <button
                              onClick={() =>
                                handleQuantityChange(item.skuId, (item.quantity || 0) - 1)
                              }
                              disabled={
                                (item.quantity || 0) <= 1 || updateCartItem.isPending
                              }
                              className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center bg-brand-tint hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
          ...
          <div>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        disabled={
                          (validateCoupon.isPending || isApplyingCoupon) || !couponCode.trim()
                        }
                        className="btn-primary"
                      >
                        {(validateCoupon.isPending || isApplyingCoupon)
                          ? 'Applying...'
                          : 'Apply'}
                      </button>
                    </div>
                ...
                {appliedCoupon ? (
                  <>
                    <div className="flex justify-between text-sm text-green-600">
                      <span>
                        Discount (KSh {discountAmount.toFixed(2)} /{' '}
              ...
              <button
                onClick={() => navigate('/products')}
                className="w-full btn-secondary mt-3 py-3"
              >
                Continue Shopping
              </button>
            </div>
